...
- (void)init:(NSMutableArray*)inArguments{
    ...
    void (^callback)(BOOL result) = ^(BOOL result){
        ...
        UEX_ERROR err = result ? kUexNoError : uexErrorMake(1);
...
- (void)cbStartRoutePlanWithError:(UEX_ERROR)error dataDict:(NSDictionary *)dict{
    [self.webViewEngine callbackWithFunctionKeyPath:@"uexBaiduNavi.cbStartRoutePlan" arguments:ACArgsPack(dict.ac_JSONFragment)];
    [self.cbStartRoutePlan executeWithArguments:ACArgsPack(error,dict)];
    self.cbStartRoutePlan = nil;
}
...
- (void)routePlanDidFinished:(NSDictionary *)userInfo{
    ...
    [self cbStartRoutePlanWithError:kUexNoError dataDict:dict];
}
...
- (void)routePlanDidFailedWithError:(NSError *)error andUserInfo:(NSDictionary *)userInfo{
    ...
    switch ([error code]) {
        case BNAVI_ROUTEPLAN_ERROR_LOCATIONFAILED:
            errorCode = 1;
            break;
        case BNAVI_ROUTEPLAN_ERROR_NONETWORK:
        case BNAVI_ROUTEPLAN_ERROR_SEARCHFAILED:
        case BNAVI_ROUTEPLAN_ERROR_NETWORKABNORMAL:
        case BNAVI_ROUTEPLAN_ERROR_ROUTEPLANFAILED:
        case BNAVI_ROUTEPLAN_ERROR_SETSTARTPOSFAILED:
        case BNAVI_ROUTEPLAN_ERROR_SETENDPOSFAILED:
        case BNAVI_ROUTEPLAN_ERROR_DATANOTREADY:
        case BNAVI_ROUTEPLAN_ERROR_ENGINENOTINIT:
            errorCode = 2;
            break;
        ...
        case BNAVI_ROUTEPLAN_ERROR_NODESTOONEAR:
            errorCode = 4;
            break;
        case BNAVI_ROUTEPLAN_ERROR_INVALIDSTARTENDNODE:
        case BNAVI_ROUTEPLAN_ERROR_INPUTERROR:
            errorCode = 5;
            break;
        case BNAVI_ROUTEPLAN_ERROR_WAITAMOMENT:
            errorCode = 6;
        default:
            break;
    }
    ...
    [dict setValue:userInfo forKey:@"extras"];
    [self cbStartRoutePlanWithError:uexErrorMake(errorCode) dataDict:dict];
}
...
- (void)routePlanDidUserCanceled:(NSDictionary*)userInfo {
    ...
    [dict setValue:userInfo forKey:@"extras"];
    [self cbStartRoutePlanWithError:uexErrorMake(1) dataDict:dict];
}
...
- (void)exitNavi:(NSMutableArray *)inArguments{
    ACArgsUnpack(NSDictionary *info) = inArguments;
    NSDictionary* extras = dictionaryArg(info[@"extras"]);
    [BNCoreServices_UI exitPage:EN_BNavi_ExitAllVC animated:YES extraInfo:extras];
}
